...
#' Initialises the RScala instance to run scCross
#' 
#' @return An instance of RScala for scCross
#' 
initRscala = function() {
  scX.rs = scala()
  ...
  scX.rs + processFile("~/Documents/implementations/scCross/src/main/scala/scXObjective.scala")
  scX.rs + processFile("~/Documents/implementations/scCross/src/main/scala/scXBeam.scala")
  scX.rs + processFile("~/Documents/implementations/scCross/src/main/scala/scXLocalSearch.scala")
  scX.rs
}



#' Computes the sum of positive expression for each gene in the transformed data for each sample
#'
#' @param data The (normalized) count data (will be transformed inside the function according to \code{data.tfo})
#' @param sample.info The sample id of each cell
...
#' @param cells.on.col \code{TRUE} if data is GENES x CELLS (default), \code{FALSE} if CELLS x GENES
#' 
#' @return A matrix with one row for each sample, containing the values for each gene
#' 
geneSumPerSample = function(data, sample.info, data.tfo = "log10", cells.on.col = TRUE) {
  ...
  if (cells.on.col) {
    cpe.vec = c()
    for (s in unique(sample.info)) {
      cpe.vec = c(cpe.vec, as.vector(apply(data[, sample.info == s], 1, count_cpe)))
    }
    cpe.sam = matrix(cpe.vec, ncol = nrow(data), byrow = T)
  } else {
    cpe.vec = c()
    for (s in unique(sample.info)) {
      cpe.vec = c(cpe.vec, as.vector(apply(data[, sample.info == s], 2, count_cpe)))
    }
    cpe.sam = matrix(cpe.vec, ncol = ncol(data), byrow = T)
  }
  cpe.sam
}
...
#'
#' @param scX.rs A scCross RScala instance
#' @param data.matrix A CELLS x GENES expression matrix (with positive and negative values)
#' @param sam.id.cells The sample id of each cell (ids should be 0, 1, 2, etc.)
#' @param gene.sum.per.sample The sum of positive expression in \code{data.matrix} for each gene (computed if not given)
...
#' @param k.adapt Whether to adapt kappa if too many/few markers are selected (default: \code{FALSE})
...
#' 
runScX.beam = function(scX.rs, data.matrix, sam.id.cells, gene.sum.per.sample, rareness.score = c(), nNeg = 0.1, kappa = 1.0, k.adapt = FALSE, n.heur.pair = 100, n.heur.keep = 100, nh.adapt = TRUE, max.nb.cells = Inf, stop.no.improve = 25, verbose = TRUE) {
  
  if (length(rareness.score) == nrow(data.matrix)) {
    res = scX.rs(m = data.matrix, sic = sam.id.cells, gs = gene.sum.per.sample, rs = rareness.score, nn = nNeg, k = kappa, ka = k.adapt, nhp = n.heur.pair, nhk = n.heur.keep, nha = nh.adapt, mc = max.nb.cells, sni = stop.no.improve, v = verbose) *
      'scXBeam.findCluster(m, samIdCells = sic.map(_.toInt), preGeneSumPerSample = gs, rarenessScore = rs.toArray, nNeg = nn, kappa = k, kAdapt = ka, nHeurPair = nhp.toInt, nHeurKeep = nhk.toInt, nhAdapt = nha, maxNbCells = mc.toInt, stopNoImprove = sni.toInt, verbose = v)'
  } else {
    res = scX.rs(m = data.matrix, sic = sam.id.cells, gs = gene.sum.per.sample, nn = nNeg, k = kappa, ka = k.adapt, nhp = n.heur.pair, nhk = n.heur.keep, nha = nh.adapt, mc = max.nb.cells, sni = stop.no.improve, v = verbose) *
      'scXBeam.findCluster(m, samIdCells = sic.map(_.toInt), preGeneSumPerSample = gs, nNeg = nn, kappa = k, kAdapt = ka, nHeurPair = nhp.toInt, nHeurKeep = nhk.toInt, nhAdapt = nha, maxNbCells = mc.toInt, stopNoImprove = sni.toInt, verbose = v)'
  }
  
  clust.cells = scX.rs(res = res) * 'res._1.map(_ + 1).toArray'
  clust.genes = scX.rs(res = res) * 'res._2.map(_ + 1).toArray'
  clust.obj = scX.rs(res = res) * 'res._3'
  kappa.final = scX.rs(res = res) * 'res._4'
...
#' 
runScX.local = function(scX.rs, data.matrix, sam.id.cells, init.sol, gene.sum.per.sample, kappa = 1.0, nNeg = 0.1, nb.iter = 1000, nb.restart = 20, seed = 0, verbose = TRUE) {
  
  res = scX.rs(m = data.matrix, sic = sam.id.cells, is = init.sol, gs = gene.sum.per.sample,  k = kappa, nn = nNeg, ni = nb.iter, nr = nb.restart, s = seed, v = verbose) *
    'scXLocalSearch.localSearch(m, samIdCells = sic.map(_.toInt), initCells = is.map(_ - 1).toSet, preGeneSumPerSample = gs.toArray, kappa = k, nNeg = nn, nbIter = ni.toInt, nbRestart = nr.toInt, seed = s.toInt, verbose = v)'
...
#' @param sample.info The sample id of each cell
#' @param gene.sum.per.sample The sum of positive expression in the transformed data for each gene, per sample (computed if not given)
...
#' @param nNeg The maximum proportion of negative values allowed inside the cluster (default: 0.1)
#' @param n.adapt Whether to adapt nNeg before local search (default: \code{FALSE})
#' @param kappa The out-of-cluster expression penalty constant (default: "auto", i.e. computed as 100 / nb. cells)
#' @param k.adapt Whether to adapt kappa if too many/few markers are selected during breadth-first search (default: \code{FALSE})
#' @param min.cells.beam The minimum number of cells to use in the beam search, when a \code{rareness.score} is provided (default: 1000)
...
#' @param verbose enable/disable printing (default: \code{TRUE})
runScCross = function(scX.rs, data, sample.info, gene.sum.per.sample = c(), rareness.score = c(), data.tfo = "log10", cells.excl = c(), genes.excl = c(), nNeg = 0.1, n.adapt = FALSE, kappa = "auto", k.adapt = FALSE, min.cells.beam = 1000, iqr = 1.5, cells.on.col = TRUE, seed = 0, verbose = TRUE) {
  if (kappa == "auto"  && cells.on.col) {
    kappa = round(100 / ncol(data), 3)
  } else if (kappa == "auto" && !cells.on.col) {
    kappa = round(100 / nrow(data), 3)
  }
  
  samples = unique(sample.info)
  sam.id.cells = rep(0, length(sample.info))
  for (i in 1 : length(samples)) {
    sam.id.cells[sample.info == samples[i]] = i - 1
  }
  sam.id.cells = as.numeric(sam.id.cells)
  
  if (nrow(gene.sum.per.sample) != length(samples) || (ncol(gene.sum.per.sample) != nrow(data) && ncol(gene.sum.per.sample) != ncol(data))) {
    gene.sum.per.sample = geneSumPerSample(data, sam.id.cells, data.tfo, cells.on.col)
  }
  ...
  if (cells.on.col) {
    res.1st = runScX.beam(scX.rs, data.tfo(as.matrix(t(data[genes.idx.1st, cells.idx.1st]))), sam.id.cells[cells.idx.1st], as.matrix(gene.sum.per.sample[, genes.idx.1st]), rareness.score[cells.idx.1st], nNeg = nNeg, kappa = kappa, k.adapt = k.adapt, verbose = verbose)
  } else {
    res.1st = runScX.beam(scX.rs, data.tfo(as.matrix(data[cells.idx.1st, genes.idx.1st])), sam.id.cells[cells.idx.1st], as.matrix(gene.sum.per.sample[, genes.idx.1st]), rareness.score[cells.idx.1st], nNeg = nNeg, kappa = kappa, k.adapt = k.adapt, verbose = verbose)
  }
  ...
  if (cells.on.col) {
    res.2nd = runScX.local(scX.rs, data.tfo(as.matrix(t(data[genes.idx.2nd, cells.idx.2nd]))), sam.id.cells[cells.idx.2nd], prev.sol, as.matrix(gene.sum.per.sample[, genes.idx.2nd]), kappa = res.1st$kappa.final, nNeg = new.nNeg, nb.iter = length(cells.idx.2nd) * 100, nb.restart = 20, seed = seed, verbose = verbose)
  } else {
    res.2nd = runScX.local(scX.rs, data.tfo(as.matrix(data[cells.idx.2nd, genes.idx.2nd])), sam.id.cells[cells.idx.2nd], prev.sol, as.matrix(gene.sum.per.sample[, genes.idx.2nd]), kappa = res.1st$kappa.final, nNeg = new.nNeg, nb.iter = length(cells.idx.2nd) * 100, nb.restart = 20, seed = seed, verbose = verbose)
  }
...
runScCross.quick = function(scX.rs, data, sample.info, init.cells, init.genes, gene.sum.per.sample = c(), data.tfo = "log10", cells.excl = c(), genes.excl = c(), nNeg = 0.1, kappa = "auto", cells.on.col = TRUE, nb.restart = 20, seed = 0, verbose = TRUE) {
  if (kappa == "auto"  && cells.on.col) {
    kappa = round(100 / ncol(data), 3)
  } else if (kappa == "auto" && !cells.on.col) {
    kappa = round(100 / nrow(data), 3)
  ...
  if (cells.on.col) {
    res = runScX.local(scX.rs, data.tfo(as.matrix(t(data[genes.idx, cells.idx]))), sam.id.cells[cells.idx], prev.sol, as.matrix(gene.sum.per.sample[, genes.idx]), kappa = kappa, nNeg = nNeg, nb.iter = length(cells.idx) * 100, nb.restart = nb.restart, seed = seed, verbose = verbose)
  } else {
    res = runScX.local(scX.rs, data.tfo(as.matrix(data[cells.idx, genes.idx])), sam.id.cells[cells.idx], prev.sol, as.matrix(gene.sum.per.sample[, genes.idx]), kappa = kappa, nNeg = nNeg, nb.iter = length(cells.idx) * 100, nb.restart = nb.restart, seed = seed, verbose = verbose)
  }
